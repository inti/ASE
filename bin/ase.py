...
df_count_data_unique.loc[:,"pASE_1"] = df_count_data_unique.progress_apply(lambda x: prob_1_diff_2(x['alpha_post'],x['beta_post'],null_pars[0],null_pars[1],invert_to_seep_up=True, pseucount=0),axis=1)
...
df_count_data_unique.loc[:,"pASE_2"] = df_count_data_unique.progress_apply(lambda x: prob_ASE_mixt_prior(x['alpha_post'],x['beta_post'],pars, null_pars=null_pars, prior_w=prior_w,invert_to_seep_up=True, pseucount=0),axis=1)
# correct cases where prob is inf or > 1
